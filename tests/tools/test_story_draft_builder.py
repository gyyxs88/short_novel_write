...
def test_build_story_markdown_from_payload_returns_full_markdown() -> None:
    payload = build_story_payload(plan=PLAN)
    draft = build_story_markdown_from_payload(payload)

    assert draft["title"] == "短信背后的真相"
    assert "## 简介" in draft["content_markdown"]
    assert "## 正文" in draft["content_markdown"]
    assert "### 1" in draft["content_markdown"]
    assert "### 6" in draft["content_markdown"]
    assert draft["body_char_count"] >= 4500
    assert len(draft["chapters"]) == 6
    assert "章节里要持续强化" not in draft["content_markdown"]
    assert "推进事件，并把" not in draft["content_markdown"]
    assert "回收“" not in draft["content_markdown"]


def test_build_story_markdown_from_payload_keeps_summary_in_range_for_long_fields() -> None:
    payload = build_story_payload(plan=PLAN)
    payload["core_relationship"] = "主角与那个牵出前任旧账的人，重新站到了彼此试探的位置。"
    payload["main_conflict"] = "为了查清旧案背后的操盘逻辑，主角必须在婚礼前夜先保住自己。"
    payload["key_turning_point"] = "主角发现最可靠的一条证据，其实是有人故意递到她手里的诱饵。"

    draft = build_story_markdown_from_payload(payload)
    structure = check_story_markdown(
        draft["content_markdown"],
        target_char_range=(5000, 8000),
        summary_char_range=(50, 120),
    )

    assert structure.summary_chars >= 50
    assert structure.summary_chars <= 120
    assert structure.is_valid is True


def test_build_story_markdown_from_payload_is_structurally_valid_and_quality_passable() -> None:
    payload = build_story_payload(plan=PLAN)
    draft = build_story_markdown_from_payload(payload)
    ...
    quality = check_story_quality_markdown(draft["content_markdown"])

    assert structure.is_valid is True
    assert quality.is_passable is True
