...
18. 正文气味分析与结果落库
   通过 `tools/story_prose_analyzer.py` 对正文做 deterministic 文本气味诊断，当前覆盖重复短语、段落起手重复、AI-ism、抽象情绪、场景稀薄和章节模板感，并补充了只做提醒、不直接进改写主通道的 `risk_signals` 层，用来标记眼神套路句、空泛态度词、偷懒判断句、整齐平衡句、动物比喻和冲击型套比喻等高风险表达；分析结果可落库到 `story_draft_analyses`。
19. 风格画像构建与管理
...
**一个已经把“创意 -> 方案 -> payload -> 正文 -> 自动修订 -> inspect -> save/archive”主链路接通的短篇写作仓库；当前重点已不是补齐流程，而是继续做正文质量稳定性和 LLM 生产化验收。**
...
如果你期待的是“直接运行一个命令，就稳定产出高质量、可直接发布的完整短篇”，那现在还不能这样承诺。  
但如果你要的是“把创意、方案、payload、正文、自动修订、inspect、save / archive 串成一条可执行链路”，当前仓库已经能做到：主 skill 已接通，deterministic 基线已能端到端跑通，部分 LLM 样本也已经完成真实链路验收。  
当前真正还在继续补的，不是“正文生成功能有没有接上”，而是正文质量稳定性、自动修订可靠性，以及更大范围的 LLM 生产化验收。
