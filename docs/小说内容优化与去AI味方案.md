...
后续如果继续吸收外部“去 AI 味”经验，默认按下面这套口径执行：

1. 不照单全收，不搞一刀切硬禁。
2. 先把它们当成“风险信号”，不是“命中即错”。
3. 只有高信号、低歧义、适合工程落地的表达，才进入 analyzer 词表或画像 `avoid_phrases`。
4. 不把中文正常语法和常规修辞整体打成问题，例如不能直接硬禁所有比喻、形容词、副词、连词、平衡句。

当前适合吸收的重点，主要是这些高风险表达类型：

- AI 腔胶水词和空泛补缀词，例如：`某种`、`带着某种`、`带着一种`、`某种意味`、`并没有立刻`
- 抽象情绪直说，例如直接宣告“痛苦”“不安”“关系开始松动”，却没有动作、对话或场景支撑
- 偷懒判断句，例如“不容置疑”“难以察觉”这类把判断直接说死的表达
- 套路化神态和眼神描写，例如“眼底闪过”“眼神闪过”“闪过一丝……”
- 过度整齐的平衡句和总结句，例如连续出现“不是……而是……”并承担解释性收束
- 动物比喻和冲击型套比喻，例如：`像一只……`、`像投入湖面的巨石`、`泛起波澜`

同时要明确下面这些边界：

- 这些表达默认只作为风险提示，不作为自动判死刑的硬规则
- 高频、连用、扎堆出现时才更值得处理；偶发命中不必机械删除
- 自动改写层只处理少量相对安全的胶水表达，不强行自动改写高语义负载句
- 像“眼神闪过”“不容置疑”这类表达，更适合先提示人工复核，而不是直接做 deterministic 替换

### 2.2 命中清单的三层分工

为了避免 analyzer 一命中就被 rewriter 机械接管，后续默认按三层处理：

1. 安全自动处理层
   只放少量低歧义、低语义负载的胶水表达，例如：`某种`、`带着某种`、`并没有立刻`
2. 提醒但不自动改层
   适合放：`眼神/眼底闪过`、`带着……意味`、`不容置疑`、`不是……而是……`、`像一只……`、`巨石/惊雷/涟漪/波澜` 这类套比喻
3. 人工复核层
   适合放高依赖上下文、容易误伤风格和角色语气的表达，例如复杂比喻、带强语气的判断句、关键对白里的抽象句

工程上要坚持两条：

- 提醒层进入 `risk_signals`，不进入当前 `issues -> select_rewrite_targets` 主通道
- 只有经过样本验证、误伤率可控的表达，才允许从提醒层升级到自动处理层
